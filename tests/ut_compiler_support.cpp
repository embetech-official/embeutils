...
EMBEUTILS_STATIC_ASSERT(EMBEUTILS_ALIGNOF(AlignedStruct) == 16U, "AlignedStruct must be 16-byte aligned");
...
EMBEUTILS_NORETURN void noreturn_func() {
  for(;;) {
  }
}
...
void nonnull_func(void *ptr, void *ptr2) {
  (void)ptr;
  (void)ptr2;
}

void nonnull_second_func(void *ptr, void *ptr2) EMBEUTILS_NONNULL(2);
void nonnull_second_func(void *ptr, void *ptr2) {
  (void)ptr;
  (void)ptr2;
}
...
// ---------- EMBEUTILS_RESTRICT ----------
void restrict_copy(int *EMBEUTILS_RESTRICT dst, int const *EMBEUTILS_RESTRICT src) { *dst = *src; }
...
TEST(CompilerSupport, Noreturn) {
  ...
  // NOLINTNEXTLINE(misc-const-correctness) a function pointer has no const-qualifiable pointee
  void (*const fn)() = noreturn_func;
  (void)fn;
  SUCCEED();
}
...
TEST(CompilerSupport, Nodiscard) {
  int const val = nodiscard_func();
  EXPECT_EQ(val, 42);
}

TEST(CompilerSupport, AlwaysInline) { EXPECT_EQ(always_inline_func(5), 6); }

TEST(CompilerSupport, UnusedArg) { EXPECT_EQ(unused_arg_func(3, 99), 3); }
